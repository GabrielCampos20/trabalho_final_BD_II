...
from src.config_loader import config as CFG, PROJECT_ROOT, DISTRIBUTED_RESULTS_DIR
...
OUTPUT_DIR = DISTRIBUTED_RESULTS_DIR
...
def remover_indices(col):
    for idx in list(col.index_information().keys()):
        if idx not in ["_id_", "estado_1"]:
            try:
                col.drop_index(idx)
            except Exception as e:
                pass # Ignora caso seja um indice do sistema
...
def caso_1_busca_simples(col, barra=None, log=print):
    ...
    # --- Com Indice (Targeted via Shard Key) ---
    col.create_index([("email", ASCENDING), ("estado", ASCENDING)])
...
def caso_3_faixa_numerica(col, barra=None, log=print):
    ...
    # --- Com Indice (Targeted via Shard Key) ---
    col.create_index([("salario", ASCENDING), ("estado", ASCENDING)])
    tempos_com = medir_query(col, filtro, barra=barra)
    exp_com    = explain_query(col, filtro, "caso_1", "com_indice")
...
def caso_4_intervalo_datas(col, barra=None, log=print):
    ...
    # --- Com Indice (Targeted via Shard Key) ---
    col.create_index([("criado_em", ASCENDING), ("estado", ASCENDING)])
    tempos_com = medir_query(col, filtro, barra=barra)
    exp_com    = explain_query(col, filtro, "caso_1", "com_indice")
...
def caso_6_escrita_e_armazenamento(col, db, barra=None, log=print):
    ...
    # Insercao com todos os indices construidos previamente
    log("  Criando indices compostos (com Shard Key)...")
    from pymongo import IndexModel
    col.create_indexes([
        IndexModel([("email", ASCENDING), ("estado", ASCENDING)]),
        IndexModel([("estado", ASCENDING), ("ativo", ASCENDING)]),
        IndexModel([("salario", ASCENDING), ("estado", ASCENDING)]),
        IndexModel([("criado_em", ASCENDING), ("estado", ASCENDING)]),
        IndexModel([("estado", ASCENDING), ("bio", "text")])
    ])
...
def exportar_csv(resultados):
    ...
    caminho_agregado = os.path.join(DIR_CSV, "consultas_agregado.csv")
    ...
    caminho_bruto = os.path.join(DIR_CSV, "consultas_bruto.csv")
    ...
    if any("Caso 6" in k for k in resultados.keys()):
        caminho_escrita = os.path.join(DIR_CSV, "escrita_armazenamento.csv")
...
def main():
    ...
    log_header("=== BD2 - Experimentos Mongo Sharded Cluster ===")
    
    client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)
    db     = client[DB_NAME]
    col    = db[COL_NAME]

    if DB_NAME in client.list_database_names() and 0 < col.count_documents({}) < TAMANHO:
        print(f"\n[Setup] Banco pre-existente '{DB_NAME}' com lixo detectado. Excluindo por inteiro...")
        client.drop_database(DB_NAME)
        print("[CRITICO] Como este e um ambiente Sharded, a exclusao do banco apagou a regra de roteamento.")
        print("-> ABRORTE E EXECUTE: python scripts/init_cluster.py ANTES de rodar este script novamente.")
        exit(1)
